...
def regeln_fussboden_heizung(currtimems):
    ...
    if FBSchlimm:
        if FBVor <= FBEntwarnung:
            ...
            mqttClient.send_debug("FB nicht mehr zu heiss FBVor {} <=  FBEntwarnung {}".format(FBVor, FBEntwarnung))
            # mqttClient.set_valve("STOP")
    elif FBVor >= FBZuHeiss:
        FBSchlimm = True
        mqttClient.switchOnOff("51", "OFF")  # pumpe fb heizung
        relayBoard.slog("FB zu heiss")
        mqttClient.send_debug("FB zu heiss FBVor {} >= FBZuHeiss {}".format(FBVor, FBZuHeiss))
        ...
        while True:
            ...
            if currtime - MQTTSlowSpamPeriod > slowspamltime:
                ...
                if not FBSchlimm:
                    ...
                    mqttClient.send_debug("Sensors disabled: {}".format(dcnt))
